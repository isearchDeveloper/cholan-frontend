...
const CarDetailsTabWithImagesWithServer = async ({
    slug,
}: {
    slug: string;
}) => {
    let API1_URL = `${process.env.NEXT_PUBLIC_UAT_URL}/api/v1/${slug?.includes("-to-") ? "route" : "city" }/cars?${slug?.includes("-to-") ? "route" : "city" }=${slug}&page=${1}&limit=6`;
...
export default CarDetailsTabWithImagesWithServer;