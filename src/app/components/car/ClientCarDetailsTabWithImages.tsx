...
const CarDetailsTabWithImagesWithClient: React.FC<{ slug: string }> = ({ slug }) => {
  ...
  const fetchCarsPage = async (categorySlug: string, page: number, replace: boolean) => {
    try {
      ...
      let url = `${process.env.NEXT_PUBLIC_UAT_URL}/api/v1/${slug?.includes("-to-") ? "route" : "city" }/cars?${slug?.includes("-to-") ? "route" : "city" }=${slug}&page=${page}&limit=6`;
      if (categorySlug !== "all") {
        url += `&category=${categorySlug}`;
      }

      const resp = await axios.get<any>(url, axiosConfig);
...
export default CarDetailsTabWithImagesWithClient;
