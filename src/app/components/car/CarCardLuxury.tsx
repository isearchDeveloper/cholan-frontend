...
const CarCardLuxury = ({ data, setOpenModal }: any) => {
    ...
    <>
      <div className="w-100 p-4 overflow-hidden bg-white tourCard heritage-card p-0">
        <img
          ...
          src={data?.Luxury[0]?.primary_image || "/images/no-img.webp"}
          alt={data?.Luxury[0]?.primary_image_alt || "/images/no-img.webp"}
        />
        <div className="mt-3 pb-0">
          <h5 className="mb-2 text-black font-semibold">Luxury Fleets</h5>

          <ul className="d-flex mt-2 flex-col p-0 gap-2 car-fleet-listitems">
            {data?.Luxury?.map((data: any) => (
              <li
                key={data?.slug}
