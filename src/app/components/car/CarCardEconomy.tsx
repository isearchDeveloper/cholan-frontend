...
const CarCardEconomy = ({ data, setOpenModal }: any) => {
    ...
    <>
      <div className="w-100 p-4 overflow-hidden bg-white tourCard heritage-card p-0">
        <img
          ...
          src={data?.Economy[0]?.primary_image || "/images/no-img.webp"}
          alt={data?.Economy[0]?.primary_image_alt || "/images/no-img.webp"}
        />
        <div className="mt-3 pb-0">
          <h5 className="mb-2 text-black font-semibold">Economy Fleets</h5>

          <ul className="d-flex mt-2 flex-col p-0 gap-2 car-fleet-listitems">
            {data?.Economy?.map((data: any) => (
              <li
                key={data?.slug}
              ...
              >
                <Image
                  ...
                  sizes="100vw"
                  src="/images/tick.svg"
                  alt="/images/no-img.webp" className="custom-hover"
                />{" "}
                {data.title}
              </li>
            ))}
        ...
        <div className="mx-auto w-100 mt-3">
          <div className="button text-center">
            <button onClick={() => setOpenModal(true)} className="btn blue-btn w-100">
              Enquire Now
              <span>
                <Image
                  width={23}
