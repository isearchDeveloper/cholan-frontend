...
const BusDetailsTabWithImagesWithServer = async ({
  slug,
}: {
  slug: string;
}) => {
  let API1_URL = `${process.env.NEXT_PUBLIC_UAT_URL}/api/v1/route/buses?route=${slug}&page=${1}&limit=6`;
        ...
        <Tabs selectedIndex={0}>
          ...
          {categories.map((category: Category) => (
            <TabPanel key={category.slug}>
              <div className="row g-4">
                <div className="col-12 text-center py-5">
                  <p>No buses loaded for this category.</p>
                </div>
              </div>
            </TabPanel>
          ))}
...
export default BusDetailsTabWithImagesWithServer;