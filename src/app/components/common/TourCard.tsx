...
const TourCard: React.FC<TourCardProps> = ({
  ...
  return (
    <>
      <div className="tourCard mb-4 shadow-sm p-4 p-lg-3">
        <div className="row g-0">
          {/* <div onClick={() => redirect()} className="col-lg-3" style={{ cursor: "pointer" }}>
            {imageUrl && (
              <img
                src={validImageUrl}
                alt={title}
                className="w-100 h-100 object-cover custom-hover"
              />
            )}
          </div> */}
          {/* ✅ IMAGE (Supports JS disabled) */}
          <div className="col-lg-3" style={{ cursor: "pointer" }}>
            {jsEnabled ? (
              // ✅ JS Enabled → clickable div using router.push
              <div className="h-100" onClick={() => redirect()}>
                <img
                  src={validImageUrl}
                  alt={title}
                  className="w-100 h-100 object-cover custom-hover"
          ...
          <div className="col-lg-9">
            <div className="card-body px-lg-3 py-lg-2 pe-lg-0 mt-3 mt-lg-0">
              <div className="tour-wrapper">
                <div className="col-12 col-lg-8 ps-0">
                  <div className="tour-info pe-lg-2">
                    {/* ✅ TITLE (Supports JS disabled) */}
                    {jsEnabled ? (
                      <h3
                        onClick={() => redirect()}
                        className="card-title mb-1 fs-5 line-clamp-hotel_one"
                        style={{ cursor: "pointer" }}
                      >
                        {title}
                      </h3>
                    ) : (
                      // ✅ JS Disabled – show <h3> inside Link (so title is visible AND clickable)
                      <Link
                        href={`/packages/${slug}`}
                        className="text-black text-decoration-none"
                      >
                        <h3 className="card-title mb-1 fs-5 line-clamp-hotel_one">{title}</h3>
                      </Link>
                    )}

                    <p className="color-blue mb-0">
                      {"★".repeat(rating) + "☆".repeat(5 - rating)}
                    </p>
                    <div className="d-flex gap-5 text-muted small my-3">
                      ...
                      {tourTime && tourTime !== "null - null" && tourTime !== "undefined - undefined" ? (
                        <div className="d-flex align-items-center gap-1">
                          ...
                          <div className="d-flex flex-column ms-2">
                            <span className=" text-black">Tour Time</span>
                ...
                <div className="col-12 col-lg-4">
                  <div className="d-flex flex-lg-column align-items-end gap-2 gap-lg-2 mt-0 mt-lg-0 pb-0 pb-lg-3 h-100 justify-content-lg-center">
            ...
            <p key={index} className="list-group-item p-0 border-0 text-muted small m-0" data-aos-delay={`${1000 + index * 100}`}>
              {highlights}
            </p>
          ))} */}
                  <p className="list-group-item p-0 border-0 text-muted small m-0 line-clamp-hotel_three">
                    {highlights}
                  </p>
                </div>
              ) : null}
