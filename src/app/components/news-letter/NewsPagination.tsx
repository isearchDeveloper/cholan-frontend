...
export default function NewsPagination({
  ...
  useEffect(() => {
    setNews(initialNews);
      if (initialNews.length === limit) {
    setTotalPages(2); // next page may exist
  } else {
    setTotalPages(1); // only one page
  }
  }, [initialNews]);
  ...
  `${process.env.NEXT_PUBLIC_UAT_URL}/api/v1/news/page/list?type=${type}&page=${pageNo}&limit=${limit}`,
  ...
  const basePath = type === "news" ? "/news" : "/newsletter";
          ...
          {news.map((item: any) => (
            <article className="news-magazine-item" key={item.id}>
              ...
              <div className="news-text-wrapper beige-bg">
                ...
                <Link href={`${basePath}/${item.slug}`} className="news-btn">
                  Read More
                </Link>
              </div>
            </article>
